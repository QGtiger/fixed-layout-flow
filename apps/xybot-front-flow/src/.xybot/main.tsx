...
const _routeSyncFiles = (import.meta as any).glob('../../src/pages/**/index.tsx', {eager: true});
const _routeAsyncFiles = [];

const routeFiles = { ..._routeSyncFiles, ..._routeAsyncFiles, };
const innerRouteFiles = (import.meta as any).glob('../../src/.xybot/pages/**/index.tsx', {
  eager: true
});
...
const routes = Object.keys(allRouteFiles)
  ...
  .map((key) => {
    ...
    const Page = _routeAsyncFiles[key] ? lazy(allRouteFiles[key] as any) : allRouteFiles[key]?.default;
    const setting = (allRouteFiles[key] as any).setting || routeConfig[path] || {};
    const useLayout = !!(setting?.layout !== false && Layout);
    const fallback = setting?.fallback ?? null;
    const Comps = _routeAsyncFiles[key] ? <Suspense fallback={fallback}>
      <Page />
    </Suspense> : <Page />;
    return {
...
const router = createBrowserRouter(afterRoutes, { basename: projectProps.baseroute || microProps.baseroute });

...
import ecmascript from "./defs/ecmascript_cn.json";
// import { createApiInfoDom } from '@/components/CmEditor/CmInput/extensions/dom';
import { TypeDefinitions } from "../constant";
import { createInfoBoxRenderer } from "../completions/createInfoBoxRenderer";
...
class TernServer {
  ...
  generateAutocompletion(
    ...
    const { end: rEnd, start: rSatrt } = result;
    console.log(result);

    // @ts-expect-error
    const newCompletions = result.completions.filter((it) => !it.isKeyword);

    const completions = newCompletions.map((item) => {
      ...
      const completion: Completion = {
        ...
        apply: (view, completion, from, to) => {
          const start = rSatrt as number;
          const insert = completion.label;
          const end = rEnd as number;
      ...
      completion.info = createInfoBoxRenderer({
        name,
